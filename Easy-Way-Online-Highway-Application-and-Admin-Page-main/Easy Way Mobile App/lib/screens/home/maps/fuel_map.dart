...
class _FuelMapPageState extends State<FuelMapPage> {
  ...
  final List<LatLng> _fuelStationsAndServiceAreas = [
    ...
    const LatLng(6.821073, 79.967517), // Kottawa Entry/Exit
    const LatLng(6.703271, 79.993963), // Gelanigama Service Area
    const LatLng(6.410347, 80.577927), // Kurundugaha Service Area
    const LatLng(6.757317, 80.185971), // Matara Service Area
    const LatLng(6.226409, 80.015578), // Galle Service Area

    // Outer Circular Expressway (E02)
    const LatLng(6.866957, 80.003324), // Kadawatha Entry/Exit
    const LatLng(6.798532, 79.998254), // Kaduwela Service Area
    const LatLng(6.933920, 80.076173), // Ja-Ela Service Area

    // Colombo-Katunayake Expressway (E03)
    const LatLng(7.132385, 79.882549), // Katunayake Service Area

    // Central Expressway (E04 - Partial Completion)
    const LatLng(7.254659, 80.572507), // Meerigama Entry/Exit
    const LatLng(7.341292, 80.623614), // Kurunegala Service Area
    const LatLng(7.389323, 80.703244), // Alawwa Service Area

    // Other Major Routes (National Highways)
    const LatLng(7.873054, 80.771797), // Dambulla (A9 Junction)
    const LatLng(6.936897, 79.852179), // Wellawatte
    const LatLng(6.053519, 80.221096), // Galle
    const LatLng(8.311400, 80.403652), // Anuradhapura
    const LatLng(9.660713, 80.025501), // Jaffna
    const LatLng(7.019226, 80.218765), // Ratnapura
    const LatLng(6.932547, 80.563387), // Kandy (A1 Junction)

    // Northern Areas (A9 and others)
    const LatLng(9.084367, 80.456963), // Vavuniya Service Area
    const LatLng(9.331050, 80.468383), // Kilinochchi
    const LatLng(8.931840, 80.429950), // Mannar

    // New Fuel Stations or Service Areas (Example Locations)
    const LatLng(7.831521, 80.071356), // Example Service Area 1
    const LatLng(6.548734, 80.149215), // Example Service Area 2
  ];
